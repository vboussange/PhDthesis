...
% \end{split}
% \end{equation}
% (cf.\ \eqref{eq:defPDEspecial}).
% In \eqref{eq:plain-vanilla-SGD} in \cref{algo:special_case} we merely employ the plain vanilla stochastic gradient descent (SGD) optimisation method to train the considered neural networks (see \eqref{eq:loss_special_case} and \eqref{eq:plain-vanilla-SGD} in \cref{algo:special_case}). We refer to \eqref{eq:general_batch_normalization} and \eqref{eq:general_gradient_step} in \cref{def:general_algorithm} for a set-up covering more general SGD type optimisation methods such as the Adam optimizer (see Kingma $\&$ Ba~\cite{Kingma2014}).
% \end{comment}
