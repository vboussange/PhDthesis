...
%%
% 4th paragraph    
Here, we propose a ML framework relying on a segmentation method and on automatic differentiation and state-of-the-art optimizers to efficiently learn the parametrization of ecosystem models from observation data. 
%
...
The segmentation method partitions the data into segments, each matched against simulations of the model considered. ICs are estimated for each segment, and we show analytically how this learning strategy regularizes the ill-behaviour of the loss function arising from the strong nonlinearities of ecosystem models. 
% 
ICs are nuisance parameters that increase the number of dimensions of the optimization problem, but the efficiency of variational optimizers compensate for this additional complexity. Since segments are treated independently, independent time series can be collectively used to improve the learning. %The combination of the segmentation method, together with state-of-the-art optimizers, yield excellent calibration performance. 
% 
...
\section{Conclusion}
We proposed a ML framework based on a segmentation method combined with automatic differentiation and state-of-the-art optimizers to estimate the parameters and improve the forecast skill of complex ecosystem models from observation data.
% 
