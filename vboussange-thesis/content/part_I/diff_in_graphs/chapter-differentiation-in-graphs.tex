...
\begin{quote} %150/150 words
  \small{\textit{
	...
	%
	% Notably, habitat connectivity and heterogeneity constrain the movement and survival of individuals, thereby promoting differentiation through drift and local adaptation. 
	%
	% Nevertheless, the complexity of landscape features and of the ecological processes acting upon it can blur our understanding of how they together drive differentiation.
	%
	...
	%
	% Individuals possess neutral and adaptive traits, whose divergence results in differentiation at the population level.
	%
  ...
  % \todo[inline]{maybe add: Since natural populations are frequently subdivided, considerable effort has been devoted to extending (1) to subdivided populations. \cite{Nagylaki1998}}
  ...
  % This leads to he interplay between landscape features, dispersal, neutral and adaptive processes requires a formalised mechanistic model \cite{GARANT2007}.
  ...
  %
  In particular, these models can capture the interplay between population dynamics, spatial dynamics and phenotypic evolution, while providing a rigorous set-up for analytical investigation. % leading to a universal understanding of the system under scrutiny. 
  %
  ...
  \end{equation}
  %
  where $A = (a_{i,j})_{1\leq i, j \leq M}$ is the adjacency matrix of the graph $G$ and $D = (d_1,d_2,\dots,d_M)$ is a vector containing the degree of each vertex (number of edges incident to the vertex). % (see \nameref{secSI:supmat} for the derivation from the microscopic model).
  %
  ...
  %
  % In particular, the mean value of the populations' neutral trait evolves in a stochastic fashion because it is not influenced by a stabilising force (see \nameref{secSI:supmat}). 
  ...
  %%is mainly driven by the deterministic component of the trait dynamics. Selection, captured by the dependence of the birth rate on $\Theta_i$, stabilises the adaptive trait distribution, whose dynamics can be approximated by 
  Heterogeneous selection, captured by the dependence of the birth rate on $\Theta_i$, generates a stabilising force  that dampens the stochastic fluctuations of the adaptive trait distribution. The dynamics of the adaptive trait distribution consequently shows a deterministic behavior and we demonstrate in \nameref{secSI:supmat} and \cref{figSI:pde-vs-IBM-mresponse-setting2,figSI:pde-vs-IBM-trans-setting2_localpop} that the number of individuals on $v_i$ with traits $s \in \Omega \subset \mathcal{S}$ can be approximated by the quantity $\int_\Omega n^{(i)}(s)ds$, where $n^{(i)}$ is a continuous function solution of the PDE
  %
  ...
  % 
  % In the case where $m=0$ and assuming very rare mutations with weak effects ($\mu \sigma_\mu^2 << 1$), the local trait distributions in the stationary state are single spikes and we have that $n^{(i)} = K \delta_{\Theta_i}$ where $\delta$ is the Dirac function, so that $N^{(i)} = K$ and $\bar{s}^{(i)} = \frac{1}{N^{(i)}}\int_\mathcal{S} s \, n^{(i)}(s) ds = \Theta_i$.
  %
  % Hence under low $m$, $\mu$ and $\sigma$, population size is similar to that in setting (1) and $Q_{ST,s}$ is governed by $\Var(\Theta)$ that relates to the habitat heterogeneity.
  %
  We show how \cref{eq:PDE_adapt} can be reduced to an equivalent two-habitat model under mean field assumption. %, which reveals the importance of the quantity $r_\Theta$, the habitat assortativity, in determining the adaptive trait distribution.
  ...
  %
  % one of which is locally well-adapted while the second results from the influx of maladaptive migrants and pushes the well-adapted type away from the optimal trait value $\Theta_i$.
  %
  ...
  \begin{figure}[ht]
	\centering
	\includegraphics{main/pde_v3_horizontal.pdf}
  ...
  %%
  We extend our analyses to realistic landscapes with a continuum of habitat types by running simulations on graphs obtained from real spatial habitat datasets and by considering mean annual temperature as a proxy for habitat type (see \cref{figSI:graph_real_land,table:coefficients_realgraphs}). 
  %
  We also consider simulations accounting for trait-dependent competition to test whether our results hold under more complex ecological processes (see \nameref{secSI:supmat} for the implementation details and \cref{table:coefficients_trait-dep-comp} for the results). 
  %
  ...
  %
  Taken together, these results indicate that under sufficiently strong selection and sufficiently high habitat heterogeneity, adaptive differentiation $Q_{ST,s}$ is mainly driven by habitat assortativity $r_\Theta$. %, while the effect of homogeneity in vertex degree $h_d$ on $Q_{ST,s}$ is small and that of average path length $\l$ is non-existent. 
  ...
  \begin{figure}[ht]
	\centering
	  \includegraphics{main/sett2_adapt_IBM_horizontal_0.1.pdf}
  ...
  \begin{figure}[ht] 
	  ...
	  % 
	  % The discrepancy can be explained by the difference in average population size $\bar{N}$ between the two settings.
	  %
  ...
  %
  % This suggests that in landscapes with such features, high levels of differentiation might largely contribute to diversification overtime.
  %
  ...
  %
  % We note that obtaining analytical results for $Q_{ST,u}$ for simple cases such as e.g.\ the complete graph would be an interesting point to cover, for comparing the results with the classical model such as \cite{Kimura1964,Nagylaki1994}.
  %
  ...
  %
  % While it has been acknowledged that reaction diffusion processes behave very differently in graphs with heterogeneous connectivity compared with homogeneous graphs \cite{Colizza2007}, we further document how competitive interactions, as reaction processes, can influence phenotypic differentiation in graphs.
  %
  ...
  % The two-habitat models in \cite{Meszena1997,Debarre2013,Mirrahimi2020} corresponds to the extreme case of \cref{eq:PDE_adapt_rtheta} where $r_\Theta = -1$, but the spatial distributions of habitat type are likely to be more assortative and can therefore sustain more adaptive differentiation (\cref{fig:sett2_adapt_IBM_horizontal_0.1}).
  %
  ...
  %$r_\Theta$ can be obtained for general habitat distributions that might not be neither binary nor symmetric, and our conclusions hold when considering a continuum of habitat types ().
  ...
  %
  % Low $r_\Theta$ reinforces the "swamping" effect of gene flow proposed by Haldane \cite{Polechova2015} and limits local adaptation, so that $r_\Theta$, being calculated on graphs based on the dispersal range of a species, can be viewed as the spatial scale of habitat variation that conditions local adaptation \cite{Richardson2014}.
  %
  % Montane regions can support extremely varied habitats within a small spatial scale \cite{Antonelli2018}, and in this case $r_\Theta$ should be low. This implies that for taxa to be locally well-adapted, a low migration rate is required, which might explain why many species in such environments have small ranges \cite{Fjeldsa2012,Rahbek2019a}.
  %
  ...
  %
  The present approach offers a promising framework for studying complex adaptive systems, as it can elucidate how macroscopic properties emerge from microscopic processes acting upon agents structured over complex spatio-evolutionary structures. % Similar models could be used in other fields, such as linguistics \cite{Gavin2013} or economics \cite{Farmer2009}, where macroscopic properties emerge from the interactions between agents structured over complex spatio-evolutionary structures.
  ...
  %
  In the setting with heterogeneous selection, the mean field approach involves the assumption that all vertices with a similar habitat are equivalent. In this case, an individual from a vertex with habitat $\theta_{\rcirc}$ has the probability $\nicefrac{P({\bcirc},{\rcirc})}{P({\rcirc})}$ of migrating to a vertex with habitat $\theta_{\bcirc}$, and therefore \cref{eq:PDE_adapt} transforms into
  %
  \begin{equation}
  ...
  % \texttt{Rest deleted for double blind peer review.}
  
  \section{Data accessibility}
  %
  % \texttt{The code used in this article will be made available online after the review}.%at 
