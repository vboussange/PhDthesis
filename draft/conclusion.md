...
- **Alternative approaches to the mini-batch method could be considerd for the inverse modelling of eco-evolutionary dynamics.**
 ...
 - We further think that the BIC criterion may not be the most appropriate, 
  ...
  - A radically different direction proposed in \cite{Skeels} employs forward model simulations to train classifier algorithms, such as random forests and neural network, that are further used on data to further output the most likely mechanisms
